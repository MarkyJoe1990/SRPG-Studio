var SelectorEventControl = {
    ...
    getUnitUsableWeaponList: function(unit) {
        var arr = [];

        var i, currentItem, count = UnitItemControl.getPossessionItemCount(unit);
        for (i = 0; i < count; i++) {
            currentItem = UnitItemControl.getItem(unit, i);
    ...
    getStockItemList: function() {
        return this.getFilteredStockItemList(function(item) { return true });
    },

    getFilteredStockItemList: function(filterFunc) {
        var arr = [];

        var i, currentItem, count = StockItemControl.getStockItemCount();
        for (i = 0; i < count; i++) {
            currentItem = StockItemControl.getStockItem(i);
            filterFunc(currentItem) === true && arr.push(currentItem);
        }
    ...
    getItemListFromIdArray: function(idArray, isRandomized, goalLength) {
        ...
        for (i = 0; i < count; i++) {
            ...
            // convert to a real item.
            currentItem = ExpandedItemControl.getItemFromId(currentId);
    ...
    getParameterListFromIdArray: function(idArray, isRandomized, goalLength) {
        return this.getListFromIdArray(idArray, "parameter", isRandomized, goalLength);
    },
    ...
    _configureSelectorDataArray: function(groupArray) {
        ...
        groupArray.push(SelectorData.Parameter);
...
SelectorData.Item = defineObject(BaseSelectorData, {
    ...
    getObjectId: function() {
        ...
        return ExpandedItemControl.getConvertedItemId(object);
    },
...
SelectorData.Weapon = defineObject(SelectorData.Item, {
    ...
    getSessionDataList: function() {
        ...
        return SelectorEventControl.getUnitUsableWeaponList(unit);
    }
});
...
SelectorData.Parameter = defineObject(BaseSelectorData, {
    getSelectorDataName: function() {
        return "parameter";
    },

    getDefaultDataList: function() {
        var arr = ParamGroup._objectArray;
        var list = StructureBuilder.buildDataList();

        // Change the ID get method to grab the object's Parameter Type
        list.getDataFromId = function(id) {
            var i, currentObject, count = this._arr.length;
            for (i = 0; i < count; i++) {
                currentObject = this.getData(i);

                if (currentObject.getParameterType() === id) {
                    return currentObject;
                }
            }
        ...
        var i, currentParameter, count = arr.length;
        for (i = 0; i < count; i++) {
            currentParameter = arr[i];
            if (currentParameter.isParameterDisplayable() === true) {
                newArr.push(currentParameter);
            }
        }
    ...
    getSessionDataList: function() {
        return this.getDefaultDataList();
    },
    ...
    getObjectId: function() {
        ...
        return object.getParameterType();
    },
    ...
    _getWindowManager: function() {
        return SelectorEventParameterWindowManager;
    }
});
